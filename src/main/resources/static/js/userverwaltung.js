/**
 * view-controller for userverwaltung.html
 *
 * FiBu-Trainer
 *
 * @author Mia Gudelj
...
$(document).ready(
    function () {
        ...
        /**
         * listener for buttons within klasseForm
         */
        $("#klasseForm").on("click", "button", function () {
    ...
    $.ajax({
        ...
        .fail(function (xhr, status, errorThrown) {
            if (xhr.status == 403) {
                ...
                $("#message").text("Fehler beim Lesen der Klasse");
            }
        })
}
...
function showKlasse(klasseData) {
    ...
    $.each(klasseData, function (klasseUUID, klasse) {
        // TODO usertype?
        if (Cookies.get("userType") == "ADMIN") {
            ...
            blockData += `                <a href='./klasseEdit.html?klasseUUID=" + klasseUUID + "'>`;
            ...
            blockData += `                    <a class='btn' role='button' href='./klasseEdit.html?klasseUUID=" + klasseUUID + "'>`;
            ...
            blockData += `            </div>`;
            blockData += `            <div class="col-2">`;
            blockData += `                <button class='btn delete' id='deleteButton" + klasseUUID + "' value='" + klasseUUID + "' title='Klasse löschen'>`;
 ...
 * @param klasseUUID
 */
function deleteKlasse(klasseUUID) {
    $.ajax({
        // TODO richtige url korrigieren
        url: "klasse/delete?klasseUUID=" + klasseUUID,
        dataType: "text",
        type: "DELETE",
    })
